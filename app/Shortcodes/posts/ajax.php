...
namespace DMS\Shortcodes\posts;
...
function load_posts() {
	...
	$shortcode_atts = \DMS\Helper\Utils::sanitize_array_text_params( $shortcode_atts );
	...
	$query_vars = \DMS\Helper\Utils::sanitize_array_text_params( $query_vars );

	// query for posts
	$query = DMS()->Model->Post->get_posts( $query_vars );

	// display posts
	while ( $query->have_posts() ): $query->the_post();

		DMS()->View->load( '/view/loop_item', array(
			'atts' => $shortcode_atts
